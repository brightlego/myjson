...
fn stringify_number(number: &f64, out_string: &mut String) {
    out_string.push_str(&number.to_string());
}
...
fn stringify_internal(value: &JSONValue, out_string: &mut String) {
    match value {
        ...
        JSONValue::Object(object) => stringify_object(object, out_string),
        JSONValue::Array(array) => stringify_array(array, out_string),
...
mod tests {
    ...
    #[test]
    fn stringify_string() {
        assert_stringify(r#""""#, String("".to_string()));
        assert_stringify(r#""abc""#, String("abc".to_string()));
        for i in 0u32..=0x1f {
            assert_stringify(&format!("\"\\u{i:0>4x}\""), String(<char>::from_u32(i).unwrap().to_string()));
    ...
    #[test]
    fn stringify_array() {
        assert_stringify("[]", Array(vec![]));
        assert_stringify("[true]", Array(vec![True]));
        assert_stringify("[true,true]", Array(vec![True, True]));
        assert_stringify("[true,true,true]", Array(vec![True, True, True]));
    }
    
    #[test]
    fn stringify_object() {
        assert_stringify(r#"{}"#, Object(FxHashMap::default()));
        assert_stringify(r#"{"a":true}"#, Object(FxHashMap::from_iter([("a".to_string(), True)])));
        assert_stringify(r#"{"a":true,"b":false}"#, Object(FxHashMap::from_iter([("a".to_string(), True),("b".to_string(), False)])));
    }
    
    #[test]
    fn stringify_number() {
        assert_stringify("0", Number(0.))
    }
}